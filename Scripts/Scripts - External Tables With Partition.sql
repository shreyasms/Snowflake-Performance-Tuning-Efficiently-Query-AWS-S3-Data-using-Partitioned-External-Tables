...
--------- External Tables with Partition -----------

create or replace external table ORDERS_WITH_PARTITION
(
 ...
 File_Partition NUMBER(38,0) as (split_part(METADATA$FILENAME,'/',2)::int)
)
...
create or replace external table CUSTOMER_WITH_PARTITION
(
 ...
 File_Partition VARCHAR(20) as (split_part(METADATA$FILENAME,'/',2)::VARCHAR)
)
PARTITION BY(File_Partition)
with 
